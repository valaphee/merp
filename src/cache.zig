    ...
    return struct {
        ...
        pub fn release(self: *Self, item: *T) void {
            ...
            while (nodeOrNull) |node| {
                ...
                const itemsBase = @intFromPtr(&node.item[0]);
                const itemsLast = @intFromPtr(&node.item[itemsPerNode - 1]);
                if (itemAddr >= itemsBase or itemAddr <= itemsLast) {
                    const nextItem: u8 = @intCast((itemAddr - itemsBase) / @sizeOf(Node));
